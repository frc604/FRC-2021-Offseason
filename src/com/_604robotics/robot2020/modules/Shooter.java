...
public class Shooter extends Module {
  ...
  private QuixSparkMAX feederMotor;

  private TalonPID shooterPID;
  private SparkPID feederPID;

  private FalconEncoder leftEncoder;

  private NEOEncoder feederEncoder;
  ...
  public Shooter() {
    ...
    feederMotor = new QuixSparkMAX(Ports.SHOOTER_FEEDER_MOTOR, "Feeder Motor", Motor.kNEO, this);

    feederMotor.resetParams();
    ...
    feederEncoder = new NEOEncoder(feederMotor);
    ...
    shooterPID =
        new TalonPID(leftMotor, leftEncoder, Calibration.Shooter.kP, 0.0, Calibration.Shooter.kD);

    feederPID = new SparkPID(feederMotor, Calibration.Feeder.kP, 0.0, Calibration.Feeder.kD);
    ...
    feederMotor.burnFlashConditionally(false);
  ...
  public class Setpoint extends Action {
    ...
    @Override
    public void run() {
      System.out.println(shooterFeedforward.calculate(setpoint.get()));
      shooterPID.setSetpointVelocity(setpoint.get(), shooterFeedforward.calculate(setpoint.get()));
      // feederPID.setSetpointVelocity(400.0, feederFeedforward.calculate(400.0));
    }
