package robotnik.prefabs.motion;
...
public class TrapezoidalMotionProfileTest {
  ...
  @BeforeEach
  void setUp() {
    this.constraints = new MotionConstraints(15.0, 50);
    var start = System.nanoTime();
    profile =
        new TrapezoidalMotionProfile(
            new MotionState(10.0, 5.0), new MotionState(24.0, 5.0), constraints);
  ...
  void graph() {
    ...
    for (double i = 0; i <= profile.getTotalTime(); i = i + 0.005) {
      xData.add(i);
      var start = System.nanoTime();
      MotionState state = profile.sample(i);
      times.add(System.nanoTime() - start);

      yData.add(state.velocity);
      yPrimeData.add(state.position);

      var factor = profile.getInverted() ? -1.0 : 1.0;

      if (i <= profile.getAccelPeriod()) {
        yDoublePrimeData.add(constraints.maxAcceleration * factor);
      } else if (profile.getAccelPeriod() < i
          && i <= (profile.getAccelPeriod() + profile.getCruisePeriod())) {
        yDoublePrimeData.add(0.0);
      } else if ((profile.getAccelPeriod() + profile.getCruisePeriod()) < i
          && i <= (profile.getTotalTime())) {
        yDoublePrimeData.add(-constraints.maxAcceleration * factor);
      }
    }
